...
function createNodesView(pb, pg){
    ...
    function get(i){
        ...
        node = {
            /*
             * currently we expose the integer ID to the public... this may be
        ...
        if(pg.dense.denseinfo){
            ...
            if(i === 0){
                ...
                node.changeset = previousNode.changeset;
                node._uid = previousNode._uid;
                node._userI = previousNode._userI;
            }

            // TODO we should test wheather adding 64bit numbers works fine with high values
            node.changeset += pg.dense.denseinfo.changeset[i].toNumber();

            // TODO we should test wheather adding 64bit numbers works fine with high values
            node._uid += pg.dense.denseinfo.uid[i];
            node.uid = '' + node._uid;

            node._userI += pg.dense.denseinfo.user_sid[i];
            node.user = pb.stringtable.getEntry(node._userI);
        }
        ...
        function readBlock(fileBlock, callback){
            ...
            return readBlob(fileBlock, function(err, blob){
                ...
                zlib.inflate(blobDataToBuffer(blob.zlib_data), function(err, data){
                    ...
                    console.log('type: ' + fileBlock.blobHeader.type);
...
function visitPrimitiveGroup(pg, opts){
    ...
    for(i = 0; i < pg.nodesView.length; ++i){
        console.log('getting node from view ' + i);
