...
function createWaysView(pb, pg){
    ...
    function get(i){
        ...
        if (way.info) {
            result.version = way.info.version;
            result.timestamp = way.info.timestamp.toNumber() * pb.date_granularity;
            result.changeset = way.info.changeset.toNumber();
            result.uid = '' + way.info.uid;
            result.user = pb.stringtable.getEntry(way.info.user_sid);
        }
