...
function createNodesView(pb, pg){
    ...
    function collectDeltaData(nodeIndex){
        var i, id, changeset, uid, userIndex, deltaData;
    ...
    function get(i){
        var id, node, previousNode, deltaData;

        deltaData = collectDeltaData(i);
        ...
        if(pg.dense.denseinfo){
            ...
            node.changeset = deltaData.changeset;
            node.uid = '' + deltaData.uid;
            node.user = pb.stringtable.getEntry(deltaData.userIndex);
        }
