...
function toArrayBuffer(buffer) {
   ...
   return buffer;
    ...
    /*
    var b = new dcodeIO.ByteBuffer();
    b.array = buffer.buffer;
    b.view = new DataView(b.array);
    b.offset = buffer.byteOffset;
...
*/
    to = new Uint8Array(src.array, srcOffset, len);
    return to;
}
    ...
    readFileBlocks(fd, function(err, fileBlocks){
        ...
        function readBlock(fileBlock, callback){
            return readBlob(fileBlock, function(err, blob){
                ...
                */
                var compressed = new Uint8Array(blob.zlib_data.toArrayBuffer());
...
function createUrlParser(opts){
    var xhr = new XMLHttpRequest();
    xhr.open('GET', opts.url || opts.filePath, true);
    xhr.responseType = 'arraybuffer';
    xhr.onerror = function(evt) {
        return opts.callback(new Error(this.statusText));
    };
    xhr.onload = function(evt) {
        createFileParser(this.response, function(err, parser){
            if(err){
